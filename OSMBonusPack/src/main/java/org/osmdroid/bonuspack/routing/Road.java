...
public class Road  implements Parcelable {
  /**
	...
	public String getmLength(Context context) {
		String result;
		if (mLength >= 100.0) {
			result = context.getString(R.string.osmbonuspack_format_distance_kilometers, (int) (mLength));
		} else if (mLength >= 1.0) {
			result = context.getString(R.string.osmbonuspack_format_distance_kilometers, Math.round(mLength * 10) / 10.0);
		} else {
			result = context.getString(R.string.osmbonuspack_format_distance_meters, (int) (mLength * 1000));
		}
		return result;
	}

	public String getmDuration(Context context) {
		String result = "";
		int totalSeconds = (int)mDuration;
		int hours = totalSeconds / 3600;
		int minutes = (totalSeconds / 60) - (hours*60);
		int seconds = (totalSeconds % 60);
		if (hours != 0) {
